...
%
% v1 - from spinS_v6
% v2 - from spinS_v7 (failsafe DOS = 0 and hist_E_Mqp1)
rng default
%
L = 8;
%
...
neighbours = '1NN';
%
REP = 1E2; % number of desired configurations per (E,M) pair
...
%
wspace_filename = ['workspace_nRPS_Ising_v2_', dim, '_', lattice_type, '_L', int2str(L), '_REP_1E', int2str(log10(REP)), '_skip_', int2str(skip)];
JDOS_filename = ['JDOS_nRPS_Ising_v2_', dim, '_', lattice_type, '_L', int2str(L), '_REP_1E', int2str(log10(REP)), '_skip_', int2str(skip)];
%
...
for q = 2:q_max
    ...
    for i = 1:N_atm
        ...
        if S_vector(i) == 1
            ...
            JDOS_nRPS(E_list == E_scan, q+1) = JDOS_nRPS(E_list == E_scan, q+1) + JDOS_nRPS(E_list == E, q);
            %
        end
        %
    end
